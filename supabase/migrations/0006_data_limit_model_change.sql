-- Client decision (2026-09-15): no cumulative daily/monthly data cap. Instead, a hard
-- 5GB ceiling per single purchase — customers buy as many separate 5GB-or-under
-- purchases as they want. Replaces the daily/monthly usage-limit enforcement that was
-- in fn_create_purchase with a simple per-transaction plan-size check.

-- No plan larger than 5GB should ever be purchasable — deactivate rather than delete,
-- since it's real seeded catalog data and deactivating preserves it if this decision
-- is ever revisited.
update public.data_plans set active = false where size_in_mb > 5000;
...
begin
  ...
  if p_type = 'data' then
    ...
    -- Hard per-transaction ceiling. No cumulative daily/monthly cap by design —
    -- customers may buy as many separate purchases as they want.
    if v_plan_mb > 5000 then
