-- Client decision (2026-09-23): a SIM stops taking orders once it has used its free transfers for
-- the month (MTN: 10 free, then a fee per transfer), instead of merely being tried last. It rests
-- until the 1st (the counter rolls over lazily by Lagos month, as before). To let a SIM keep going
-- and pay MTN's fee, raise its free_transfers_per_month.
--
-- Same signatures as 0030, so `create or replace` keeps the service_role-only grants.
